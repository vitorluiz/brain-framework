...
Exemplos:
  ...
  celebro sync all
    ...
    # add profile
    p_add = subparsers.add_parser('add', help='Adiciona um novo profile')
    p_add_prof = p_add.add_subparsers(dest='subcommand')
    p_add_prof.add_parser('profile', help='Adiciona um novo profile')
    p_add_prof.add_argument('name', nargs='?', help='Nome do profile')
    p_add.set_defaults(func=lambda args: cmd_add_profile(args))
    
    # list profiles
    p_list = subparsers.add_parser('list', help='Lista profiles')
    p_list_prof = p_list.add_subparsers(dest='subcommand')
    p_list_prof.add_parser('profiles', help='Lista todos os profiles')
    p_list.set_defaults(func=lambda args: cmd_list_profiles(args))
    
    # remove profile
    p_rem = subparsers.add_parser('remove', help='Remove um profile')
    p_rem_prof = p_rem.add_subparsers(dest='subcommand')
    p_rem_prof.add_parser('profile', help='Remove um profile')
    p_rem_prof.add_argument('name', nargs='?', help='Nome do profile')
    p_rem.set_defaults(func=lambda args: cmd_remove_profile(args))
    
    # global learn
    p_glob = subparsers.add_parser('global', help='Operacoes globais')
    p_glob_learn = p_glob.add_subparsers(dest='subcommand')
    p_glob_learn.add_parser('learn', help='Aprende conhecimento global')
    p_glob_learn.add_argument('--path', help='Caminho do arquivo/diretorio')
    p_glob_learn.add_argument('--content', help='Conteudo direto')
    p_glob_learn.add_argument('--title', help='Titulo do conhecimento')
    p_glob_learn.add_argument('--sync', action='store_true')
    p_glob_learn.add_argument('--dry-run', action='store_true')
    p_glob.set_defaults(func=lambda args: cmd_global_learn(args))
    
    # backup
    subparsers.add_parser('backup', help='Backup de todos os brains')
    
    # update
    subparsers.add_parser('update', help='Atualiza o framework')
    
    # sync all
    p_sync = subparsers.add_parser('sync', help='Sincronizacao')
    p_sync_all = p_sync.add_subparsers(dest='subcommand')
    p_sync_all.add_parser('all', help='Sync de todos os brains')
    p_sync.set_defaults(func=lambda args: cmd_sync_all(args))
    
    # admin
    p_admin = subparsers.add_parser('admin', help='Gestao de administradores')
    p_admin_cmd = p_admin.add_subparsers(dest='subcommand')
    
    p_admin_list = p_admin_cmd.add_parser('list', help='Lista administradores')
    p_admin_list.set_defaults(func=lambda args: cmd_admin_list(args))
    
    p_admin_add = p_admin_cmd.add_parser('add', help='Adiciona administrador')
    p_admin_add.add_argument('type', choices=['whatsapp', 'cli', 'grupo'], help='Tipo de admin')
    ...
    p_admin_remove = p_admin_cmd.add_parser('remove', help='Remove administrador')
